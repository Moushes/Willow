Shader "Hidden/Locked/.poiyomi/Poiyomi 8.1/Poiyomi Pro Outline/889098f2d56b78044ac6dfa6891ede52"
{
	Properties
	{
 ...
 #define PROP_GLITTERMASK 
			...
			void applyDecals(inout PoiFragData poiFragData, in PoiMesh poiMesh, in PoiCam poiCam, in PoiMods poiMods, in PoiLight poiLight)
			{
				...
				#endif
				poiFragData.baseColor.rgb = lerp(poiFragData.baseColor.rgb, customBlend(poiFragData.baseColor.rgb, decalColor.rgb, float(2)), decalColor.a * saturate(_DecalBlendAlpha2 + audioLinkDecalAlpha2));
				if (float(0))
				{
					alphaOverride += 1;
			...
			void applyMatcap(inout PoiFragData poiFragData, in PoiCam poiCam, in PoiMesh poiMesh, inout PoiLight poiLight, in PoiMods poiMods)
			{
				...
				blendMatcap(poiLight, poiFragData, float(0), float(0), float(0), float(0), float(0.444), matcap, matcapMask, float(0), float(0)
				#ifdef POI_BLACKLIGHT
				...
				blendMatcap(poiLight, poiFragData, float(0), float(0), float(0), float(0), float(0), matcap2, matcap2Mask, float(0), float(0)
				#ifdef POI_BLACKLIGHT
			...
			void ApplyRimLighting(inout PoiFragData poiFragData, in PoiMesh poiMesh, in PoiCam poiCam, in PoiLight poiLight, in PoiMods poiMods)
			{
				...
				viewDotNormal = pow(viewDotNormal, float(0.34));
				if (float(0) && float(0))
				{
				...
				float blendStrength = float(0.809);
				#ifdef POI_AUDIOLINK
				...
				#endif
				#if defined(PROP_RIMTEX) || !defined(OPTIMIZER_ENABLED)
				float4 rimColor = POI2D_SAMPLER_PAN(_RimTex, _MainTex, poiUV(poiMesh.uv[float(0)], float4(1,1,0,0)), float4(0,0,0,0)) * float4(poiThemeColor(poiMods, float4(1,0.4254881,0.4254881,1).rgb, float(0)), float4(1,0.4254881,0.4254881,1).a);
				#else
				float4 rimColor = float4(poiThemeColor(poiMods, float4(1,0.4254881,0.4254881,1).rgb, float(0)), float4(1,0.4254881,0.4254881,1).a);
				#endif
				...
				float rim = 1 - smoothstep(min(float(0.083), rimWidth), rimWidth, viewDotNormal);
				rim *= float4(1,0.4254881,0.4254881,1).a * rimColor.a * rimMask;
				if (float(0))
				{
					switch(float(0))
					{
			...
			void ApplyRimLighting(inout PoiFragData poiFragData, in PoiMesh poiMesh, in PoiCam poiCam, in PoiLight poiLight, in PoiMods poiMods)
			{
				...
				#endif
				float3 rimColor = float3(poiThemeColor(poiMods, float4(1,0.4254881,0.4254881,1).rgb, float(0)));
			...
			void applyGlitter(inout PoiFragData poiFragData, in PoiMesh poiMesh, in PoiCam poiCam, in PoiLight poiLight, in PoiMods poiMods)
			{
				...
				switch(float(0))
				{
					...
					if (float(0) > 0)
					{
						randomRotation = randomFloat3WiggleRange(randoPoint, float(0), float(0));
					}
					else
					{
					...
					finalGlitter = lerp(0, float(0.03) * glitterAlpha, glitterAlpha) + max(pow(saturate(dot(lerp(glitterReflectionDirection, poiCam.viewDir, float(0.421)), poiCam.viewDir)), float(137)), 0);
					finalGlitter *= glitterAlpha;
					break;
					case 1:
					float offset = random(randoPoint);
					float brightness = sin((_Time.x + offset) * float(0)) * float(20) - (float(20) - 1);
					finalGlitter = max(float(0.03) * glitterAlpha, brightness * glitterAlpha * smoothstep(0, 1, 1 - m_dist * float(0.08) * 10));
					break;
					case 2:
					if (float(0) > 0)
					{
					...
					finalGlitter = lerp(0, float(0.03) * glitterAlpha, glitterAlpha) + max(pow(specAngle, float(137)), 0);
				...
				float glitterMask = 1;
				#endif
				glitterMask *= lerp(1, poiLight.rampedLightMap, float(0.641));
				#ifdef POI_BLACKLIGHT
			...
			float4 frag(v2f i, uint facing : SV_IsFrontFace) : SV_Target
			{
				...
				clip(poiFragData.alpha - float(0.714));
 ...
 #define PROP_GLITTERMASK 
			...
			void applyOutlineColor(inout PoiFragData poiFragData, in PoiMesh poiMesh, in PoiLight poiLight, in PoiMods poiMods)
			{
				...
				float4 col = POI2D_SAMPLER_PAN(_OutlineTexture, _MainTex, poiUV(poiMesh.uv[float(0)], float4(1,1,0,0)), float4(0,0,0,0)) * float4(poiThemeColor(poiMods, float4(0.02207365,0.005803496,0.005027735,1).rgb, float(0)), float4(0.02207365,0.005803496,0.005027735,1).a);
