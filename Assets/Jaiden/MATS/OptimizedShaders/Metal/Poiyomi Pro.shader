Shader "Hidden/Locked/.poiyomi/Poiyomi 8.1/Poiyomi Pro/93ea10b41c4b03041896a77dc9a17ab2"
{
	Properties
	{
 ...
 #define PROP_DISSOLVENOISETEXTURE 
			...
			void applyDissolve(inout PoiFragData poiFragData, in PoiMesh poiMesh, in PoiMods poiMods)
			{
				...
				if (float(1) == 1) // Basic
				{
					...
					edgeAlpha = remapClamped(da + float(0.2), da, noise) * (1 - dissolveAlpha);
				}
				...
				if (float(0.2))
				{
					edgeColor = tex2D(_DissolveEdgeGradient, poiUV(float2(edgeAlpha, edgeAlpha), float4(1,1,0,0))) * float4(poiThemeColor(poiMods, float4(1,0.3231432,0.7605247,1).rgb, float(0)), float4(1,0.3231432,0.7605247,1).a);
					#ifndef POI_SHADOW
					...
					#endif
					poiFragData.baseColor = lerp(poiFragData.baseColor, edgeColor.rgb, smoothstep(0, 1 - float(0.62) * .99999999999, edgeAlpha));
				}
				poiFragData.emission += lerp(0, dissolveToTexture * float(0), dissolveAlpha) + lerp(0, edgeColor.rgb * float(5), smoothstep(0, 1 - float(0.62) * .99999999999, edgeAlpha));
				#endif
			}
			#endif
			//ifex float(1)==0
			...
			void applyMatcap(inout PoiFragData poiFragData, in PoiCam poiCam, in PoiMesh poiMesh, inout PoiLight poiLight, in PoiMods poiMods)
			{
				...
				blendMatcap(poiLight, poiFragData, float(0), float(0), float(0), float(1), float(0), matcap, matcapMask, float(1), float(0)
				#ifdef POI_BLACKLIGHT
			...
			void applyGlitter(inout PoiFragData poiFragData, in PoiMesh poiMesh, in PoiCam poiCam, in PoiLight poiLight, in PoiMods poiMods)
			{
				...
				switch(float(0))
				{
					...
					if (float(10) > 0)
					{
						randomRotation = randomFloat3WiggleRange(randoPoint, float(10.9), float(10));
					}
					else
					{
						randomRotation = randomFloat3Range(randoPoint, float(10.9));
					}
			...
			float4 frag(v2f i, uint facing : SV_IsFrontFace) : SV_Target
			{
				...
				float3 mainNormal = UnpackScaleNormal(POI2D_SAMPLER_PAN(_BumpMap, _MainTex, poiUV(poiMesh.uv[float(0)], float4(0,0,0,0)), float4(0,0,0,0)), float(0.25));
