Shader "Hidden/Locked/.poiyomi/Poiyomi Toon/39ec397806fd48545ad5000410835727"
{
	Properties
	{
 ...
 #define OPTIMIZER_ENABLED 
			...
			{
				float3 x0, x1, x2;
				float3 conv = lerp(GreenWrapSH(wrap), GreenWrapSHOpt(wrap), 0.148); // Should try optimizing this...
			...
			void calculateShading(inout PoiLight poiLight, inout PoiFragData poiFragData, in PoiMesh poiMesh, in PoiCam poiCam)
			{
				...
				#endif
				float shadowStrength = 0.681 * poiLight.shadowMask;
				#ifdef POI_PASS_OUTLINE
				shadowStrength = lerp(0, shadowStrength, 0.0);
				#endif
				#ifdef _LIGHTINGMODE_WRAPPED
				#define GREYSCALE_VECTOR float3(.33333, .33333, .33333)
				float3 directColor = _LightColor0.rgb * saturate(RTWrapFunc(poiLight.nDotL, 1.1, 0.148));
				float3 indirectColor = 0;
				#ifdef UNITY_PASS_FORWARDBASE
				indirectColor = ShadeSH9_wrapped(lerp(poiMesh.normals[0], poiMesh.normals[1], 0.0), 1.1) * poiLight.occlusion;
				#endif
				...
				poiLight.rampedLightMap = saturate((poiLight.lightMap - (1 - 0.494)) / saturate((1 - 0.102) - (1 - 0.494) + fwidth(poiLight.lightMap)));
				float3 mathRamp = lerp(float3(1, 1, 1), saturate(lerp((float4(0.005536096,0.005536096,0.005536096,1) * lerp(indirectColor, 1, 1.0)), float3(1, 1, 1), saturate(poiLight.rampedLightMap))), 0.681);
				directColor *= saturate(poiLight.rampedLightMap + 1 - 0.681) * float4(1,1,1,1);
				float3 finalWrap = directColor + indirectColor;
				if (1.0)
				{
					finalWrap = clamp(finalWrap, 1.0, 1.51);
				}
				else
				{
					finalWrap = max(finalWrap, 1.0);
				}
				poiLight.finalLighting = finalWrap * saturate(mathRamp + 1 - 0.681);
				#endif
			...
			void applyMatcap(inout PoiFragData poiFragData, in PoiCam poiCam, in PoiMesh poiMesh, inout PoiLight poiLight, in PoiMods poiMods)
			{
				...
				blendMatcap(poiLight, poiFragData, poiMods, 0.0, 0.0, 0.0, 0.828, 0.0, 0.0, matcap, matcapMask, 0.0, 0.0, 0.0, 1.0, matcapALD);
				#endif
			}
			#endif
			#ifdef _CUBEMAP
			...
			void MochieBRDF(inout PoiFragData poiFragData, in PoiCam poiCam, inout PoiLight poiLight, in PoiMesh poiMesh, inout PoiMods poiMods)
			{
				...
				specular = poiLight.directColor * specularTerm * fresnelTerm * specularMask * poiThemeColor(poiMods, float4(0.7433497,0.7433497,0.7433497,1), 0.0) * poiLight.occlusion * attenuation;
				if (poiFragData.toggleVertexLights)
				{
					#if defined(VERTEXLIGHT_ON)
					for (int index = 0; index < 4; index++)
					{
						...
						vSpecular += poiLight.vColor[index] * specularTerm * fresnelTerm * specularMask * poiThemeColor(poiMods, float4(0.7433497,0.7433497,0.7433497,1), 0.0) * poiLight.occlusion;
					}
					#endif
				}
				if (0.0 == 1)
				{
					...
					specular2 = poiLight.directColor * specularTerm * fresnelTerm * specularMask * poiThemeColor(poiMods, float4(0.7433497,0.7433497,0.7433497,1), 0.0) * poiLight.occlusion * attenuation * 1.0;
					if (poiFragData.toggleVertexLights)
					{
						#if defined(VERTEXLIGHT_ON)
						for (int index = 0; index < 4; index++)
						{
							...
							vSpecular2 += poiLight.vColor[index] * specularTerm * fresnelTerm * specularMask * poiThemeColor(poiMods, float4(0.7433497,0.7433497,0.7433497,1), 0.0) * poiLight.occlusion * 1.0;
						}
						#endif
					}
				}
				...
				reflections *= poiThemeColor(poiMods, float4(0.7433497,0.7433497,0.7433497,1), 0.0);
			...
			void CalculateUTSSpecular(inout PoiFragData poiFragData, in PoiCam poiCam, inout PoiLight poiLight, in PoiMesh poiMesh, in PoiMods poiMods)
			{
				...
				{
					specMask1 += poiEdgeNonLinear(specArea, (1.0 - pow(0.2, 5)), 0.016) * 1.0;
				...
				{
					case 0:
					poiFragData.baseColor = lerp(poiFragData.baseColor, specularMap * poiThemeColor(poiMods, float4(0.7077566,0.7077566,0.7077566,1), 0.0) * lerp(1, poiLight.directColor, 1.0), finalSpecMask);
					break;
					case 1:
					poiLight.finalLightAdd += max(0, specularMap * poiThemeColor(poiMods, float4(0.7077566,0.7077566,0.7077566,1), 0.0) * lerp(1, poiLight.directColor, 1.0) * finalSpecMask);
					break;
					case 2:
					poiFragData.baseColor = lerp(poiFragData.baseColor, blendScreen(poiFragData.baseColor, specularMap * poiThemeColor(poiMods, float4(0.7077566,0.7077566,0.7077566,1), 0.0) * lerp(1, poiLight.directColor, 1.0)), finalSpecMask);
					break;
					case 3:
					poiFragData.baseColor = lerp(poiFragData.baseColor, poiFragData.baseColor * specularMap * poiThemeColor(poiMods, float4(0.7077566,0.7077566,0.7077566,1), 0.0) * lerp(1, poiLight.directColor, 1.0), finalSpecMask);
					break;
				}
				float3 vSpecMask = 0;
				if (poiFragData.toggleVertexLights)
				{
					...
					for (int index = 0; index < 4; index++)
					{
						...
						{
							vSpecMask = poiEdgeNonLinear(specArea, (1.0 - pow(0.2, 5)), 0.016) * 1.0 * poiLight.vAttenuation[index];
						...
						{
							case 0:
							poiFragData.baseColor = lerp(poiFragData.baseColor, specularMap * poiThemeColor(poiMods, float4(0.7077566,0.7077566,0.7077566,1), 0.0) * lerp(1, poiLight.vColor[index], 1.0), finalSpecMask);
							break;
							case 1:
							poiLight.finalLightAdd += max(0, specularMap * poiThemeColor(poiMods, float4(0.7077566,0.7077566,0.7077566,1), 0.0) * lerp(1, poiLight.vColor[index], 1.0) * finalSpecMask);
							break;
							case 2:
							poiFragData.baseColor = lerp(poiFragData.baseColor, blendScreen(poiFragData.baseColor, specularMap * poiThemeColor(poiMods, float4(0.7077566,0.7077566,0.7077566,1), 0.0) * lerp(1, poiLight.vColor[index], 1.0)), finalSpecMask);
							break;
							case 3:
							poiFragData.baseColor = lerp(poiFragData.baseColor, poiFragData.baseColor * specularMap * poiThemeColor(poiMods, float4(0.7077566,0.7077566,0.7077566,1), 0.0) * lerp(1, poiLight.vColor[index], 1.0), finalSpecMask);
							break;
						}
					}
					#endif
			...
			float4 frag(VertexOut i, uint facing : SV_IsFrontFace) : SV_Target
			{
				...
				if (1.0)
				{
					poiLight.directColor = min(poiLight.directColor, 1.51);
					poiLight.indirectColor = min(poiLight.indirectColor, 1.51);
				}
				if (0.0)
				{
