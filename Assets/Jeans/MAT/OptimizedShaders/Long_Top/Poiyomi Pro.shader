Shader "Hidden/Locked/.poiyomi/Poiyomi 8.1/Poiyomi Pro/b2ef6f0ebae281748a9f4dd9ff9cbcce"
{
	Properties
	{
		...
		_DissolveAlpha_Long_Top ("Dissolve Alpha", Range(0, 1)) = 0
 ...
 #define PROP_DISSOLVENOISETEXTURE 
			...
			void applyDissolve(inout PoiFragData poiFragData, in PoiMesh poiMesh, in PoiMods poiMods)
			{
				...
				#endif
				#if defined(PROP_DISSOLVENOISETEXTURE) || !defined(OPTIMIZER_ENABLED)
				float dissolveNoiseTexture = POI2D_SAMPLER_PAN(_DissolveNoiseTexture, _MainTex, poiUV(poiMesh.uv[float(0)], float4(3,3,0,0)), float4(0,0,0,0)).r;
				#else
				float dissolveNoiseTexture = 1;
				#endif
				float da = _DissolveAlpha_Long_Top
				+ float(0)
				+ float(0)
				+ float(0)
				+ float(0)
			...
			void applyMatcap(inout PoiFragData poiFragData, in PoiCam poiCam, in PoiMesh poiMesh, inout PoiLight poiLight, in PoiMods poiMods)
			{
				...
				blendMatcap(poiLight, poiFragData, float(0.012), float(0), float(0), float(0), float(0), matcap, matcapMask, float(0), float(0)
				#ifdef POI_BLACKLIGHT
