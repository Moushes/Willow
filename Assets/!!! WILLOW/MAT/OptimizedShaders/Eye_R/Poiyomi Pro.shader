Shader "Hidden/Locked/.poiyomi/Poiyomi 8.1/Poiyomi Pro/4c2ce5a89a412fa4cb003d59164f852c"
{
	Properties
	{
		[HideInInspector] shader_master_label ("<color=#E75898ff>Poiyomi 8.1.137</color>", Float) = 0
		...
		[HideInInspector] m_start_MainHueShift_Eye_R ("Color Adjust--{reference_property:_MainColorAdjustToggle,button_help:{text:Tutorial,action:{type:URL,data:https://www.poiyomi.com/color-and-normals/color-adjust},hover:Documentation}}", Float) = 0
		...
		_Saturation_Eye_R ("Saturation", Range(-1, 10)) = 0
		...
		_MainHueShift_Eye_R ("Hue Shift--{condition_showS:(_MainHueShiftToggle==1)}", Range(0, 1)) = 0
		...
		[HideInInspector] m_end_MainHueShift_Eye_R ("Hue Shift", Float) = 0
		...
		_DecalBlendAlpha_Eye_R ("Alpha", Range(0, 1)) = 1
		...
		_DecalHueShift_Eye_R ("Hue Shift--{condition_showS:(_DecalHueShiftEnabled==1)}", Range(0, 1)) = 0
		...
		_DecalBlendAlpha1_Eye_R ("Alpha", Range(0, 1)) = 1
		...
		_DecalHueShift1_Eye_R ("Hue Shift--{condition_showS:(_DecalHueShiftEnabled1==1)}", Range(0, 1)) = 0
		...
		_DecalBlendAlpha2_Eye_R ("Alpha", Range(0, 1)) = 1
		...
		_DecalHueShift2_Eye_R ("Hue Shift--{condition_showS:(_DecalHueShiftEnabled2==1)}", Range(0, 1)) = 0
		...
		_DecalBlendAlpha3_Eye_R ("Alpha", Range(0, 1)) = 1
		...
		_DecalHueShift3_Eye_R ("Hue Shift--{condition_showS:(_DecalHueShiftEnabled3==1)}", Range(0, 1)) = 0
 ...
 #define PROP_GLITTERMASK 
			...
			float _MainHueShift_Eye_R;
			float _MainHueShiftSpeed;
			float _Saturation_Eye_R;
			...
			float _DecalBlendAlpha_Eye_R;
			float _DecalOverrideAlpha;
			float _DecalHueShiftEnabled;
			float _DecalHueShift_Eye_R;
			...
			float _DecalBlendAlpha1_Eye_R;
			float _DecalOverrideAlpha1;
			float _DecalHueShiftEnabled1;
			float _DecalHueShift1_Eye_R;
			...
			float _DecalBlendAlpha2_Eye_R;
			float _DecalOverrideAlpha2;
			float _DecalHueShiftEnabled2;
			float _DecalHueShift2_Eye_R;
			...
			float _DecalBlendAlpha3_Eye_R;
			float _DecalOverrideAlpha3;
			float _DecalHueShiftEnabled3;
			float _DecalHueShift3_Eye_R;
			...
			void applyDecals(inout PoiFragData poiFragData, in PoiMesh poiMesh, in PoiCam poiCam, in PoiMods poiMods, in PoiLight poiLight)
			{
				...
				#endif
				decalColor.rgb = decalHueShift(float(1), decalColor.rgb, _DecalHueShift_Eye_R + poiLight.nDotV * float(0), float(0));
				...
				#endif
				poiFragData.baseColor.rgb = lerp(poiFragData.baseColor.rgb, customBlend(poiFragData.baseColor.rgb, decalColor.rgb, float(0)), decalColor.a * saturate(_DecalBlendAlpha_Eye_R + audioLinkDecalAlpha0));
				if (float(0))
				{
					alphaOverride += 1;
				...
				decalColor = float4(poiThemeColor(poiMods, float4(1,1,1,1).rgb, float(0)), float4(1,1,1,1).a);
				#endif
				decalColor.rgb = decalHueShift(float(1), decalColor.rgb, _DecalHueShift1_Eye_R + poiLight.nDotV * float(0), float(0));
				...
				#endif
				poiFragData.baseColor.rgb = lerp(poiFragData.baseColor.rgb, customBlend(poiFragData.baseColor.rgb, decalColor.rgb, float(0)), decalColor.a * saturate(_DecalBlendAlpha1_Eye_R + audioLinkDecalAlpha1));
				if (float(0))
				{
					alphaOverride += 1;
				...
				decalColor = tex2D(_DecalTexture2, uv + float4(0,0,0,0) * _Time.x, ddxuv, ddyuv) * float4(poiThemeColor(poiMods, float4(1,1,1,1).rgb, float(0)), float4(1,1,1,1).a);
				#else
				uv = decalUV(float(0), float4(1,1,0,0), float4(0.5,0.5,0,0), float(0) + decalRotation, float(0), decalScale, float4(0,0,0,0) + sideMod, float(0), poiMesh, poiCam);
				decalColor = float4(poiThemeColor(poiMods, float4(1,1,1,1).rgb, float(0)), float4(1,1,1,1).a);
				#endif
				decalColor.rgb = decalHueShift(float(1), decalColor.rgb, _DecalHueShift2_Eye_R + poiLight.nDotV * float(0), float(0));
				...
				#endif
				poiFragData.baseColor.rgb = lerp(poiFragData.baseColor.rgb, customBlend(poiFragData.baseColor.rgb, decalColor.rgb, float(0)), decalColor.a * saturate(_DecalBlendAlpha2_Eye_R + audioLinkDecalAlpha2));
				if (float(0))
				{
					alphaOverride += 1;
				...
				decalColor = float4(poiThemeColor(poiMods, float4(1,1,1,1).rgb, float(0)), float4(1,1,1,1).a);
				#endif
				decalColor.rgb = decalHueShift(float(1), decalColor.rgb, _DecalHueShift3_Eye_R + poiLight.nDotV * float(0), float(0));
				...
				#endif
				poiFragData.baseColor.rgb = lerp(poiFragData.baseColor.rgb, customBlend(poiFragData.baseColor.rgb, decalColor.rgb, float(0)), decalColor.a * saturate(_DecalBlendAlpha3_Eye_R + audioLinkDecalAlpha3));
				if (float(0))
				{
					alphaOverride += 1;
			...
			{
				float2 st = frac(poiMesh.uv[float(0)] + float4(0,0,0,0).xy * _Time.x) * float(500);
				...
				switch(float(0))
				{
					...
					if (float(10) > 0)
					{
						randomRotation = randomFloat3WiggleRange(randoPoint, float(31), float(10));
					}
					else
					{
						randomRotation = randomFloat3Range(randoPoint, float(31));
					}
					float3 glitterReflectionDirection = normalize(mul(poiRotationMatrixFromAngles(randomRotation), norm));
					finalGlitter = lerp(0, float(0) * glitterAlpha, glitterAlpha) + max(pow(saturate(dot(lerp(glitterReflectionDirection, poiCam.viewDir, float(0.4)), poiCam.viewDir)), float(30)), 0);
					...
					finalGlitter = lerp(0, float(0) * glitterAlpha, glitterAlpha) + max(pow(specAngle, float(30)), 0);
					glitterColor *= poiLight.directColor;
					finalGlitter *= glitterAlpha;
					break;
				}
				glitterColor *= lerp(1, poiFragData.baseColor, float(0.999));
				#if defined(PROP_GLITTERCOLORMAP) || !defined(OPTIMIZER_ENABLED)
				...
				if (float(0) == 1)
				{
					poiFragData.baseColor = lerp(poiFragData.baseColor, finalGlitter * glitterColor * float(5), finalGlitter * glitterTexture.a * glitterMask);
					poiFragData.emission += finalGlitter * glitterColor * max(0, (float(5) - 1) * glitterTexture.a) * glitterMask;
				}
				else
				{
					poiFragData.emission += finalGlitter * glitterColor * float(5) * glitterTexture.a * glitterMask;
				}
			}
			#endif
			float4 frag(v2f i, uint facing : SV_IsFrontFace) : SV_Target
			{
				...
				if (float(1))
				{
					float shift = _MainHueShift_Eye_R;
					#ifdef POI_AUDIOLINK
				...
				poiFragData.baseColor = lerp(poiFragData.baseColor, dot(poiFragData.baseColor, float3(0.3, 0.59, 0.11)), -_Saturation_Eye_R * hueShiftAlpha.b);
