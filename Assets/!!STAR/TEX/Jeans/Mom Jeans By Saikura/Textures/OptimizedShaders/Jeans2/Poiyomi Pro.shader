Shader "Hidden/Locked/.poiyomi/Poiyomi 8.1/Poiyomi Pro/0407fa0147846b6428a1d6a8370d2c5f"
{
	Properties
	{
		...
		_DissolveAlpha_Jeans2 ("Dissolve Alpha", Range(0, 1)) = 0
 ...
 #define PROP_DISSOLVENOISETEXTURE 
			...
			void applyDecals(inout PoiFragData poiFragData, in PoiMesh poiMesh, in PoiCam poiCam, in PoiMods poiMods, in PoiLight poiLight)
			{
				...
				#endif
				poiFragData.baseColor.rgb = lerp(poiFragData.baseColor.rgb, customBlend(poiFragData.baseColor.rgb, decalColor.rgb, float(0)), decalColor.a * saturate(_DecalBlendAlpha3 + audioLinkDecalAlpha3));
				if (float(0))
				{
					alphaOverride += 1;
			...
			void applyDissolve(inout PoiFragData poiFragData, in PoiMesh poiMesh, in PoiMods poiMods)
			{
				...
				#endif
				#if defined(PROP_DISSOLVENOISETEXTURE) || !defined(OPTIMIZER_ENABLED)
				float dissolveNoiseTexture = POI2D_SAMPLER_PAN(_DissolveNoiseTexture, _MainTex, poiUV(poiMesh.uv[float(0)], float4(2,2,0,0)), float4(0,0,0,0)).r;
				#else
				float dissolveNoiseTexture = 1;
				#endif
				float da = _DissolveAlpha_Jeans2
				+ float(0)
				+ float(0)
				+ float(0)
				+ float(0)
				...
				if (float(1) == 1) // Basic
				{
					...
					edgeAlpha = remapClamped(da + float(0.016), da, noise) * (1 - dissolveAlpha);
				}
				...
				poiFragData.emission += lerp(0, dissolveToTexture * float(1), dissolveAlpha) + lerp(0, edgeColor.rgb * float(1.61), smoothstep(0, 1 - float(0) * .99999999999, edgeAlpha));
				#endif
			}
			#endif
			//ifex float(1)==0
			...
			void ApplyRimLighting(inout PoiFragData poiFragData, in PoiMesh poiMesh, in PoiCam poiCam, in PoiLight poiLight, in PoiMods poiMods)
			{
				...
				viewDotNormal = pow(viewDotNormal, float(1.48));
				if (float(0) && float(0))
				{
				...
				#endif
				#if defined(PROP_RIMTEX) || !defined(OPTIMIZER_ENABLED)
				float4 rimColor = POI2D_SAMPLER_PAN(_RimTex, _MainTex, poiUV(poiMesh.uv[float(0)], float4(1,1,0,0)), float4(0,0,0,0)) * float4(poiThemeColor(poiMods, float4(0.07417664,0.07417664,0.07417664,1).rgb, float(0)), float4(0.07417664,0.07417664,0.07417664,1).a);
				#else
				float4 rimColor = float4(poiThemeColor(poiMods, float4(0.07417664,0.07417664,0.07417664,1).rgb, float(0)), float4(0.07417664,0.07417664,0.07417664,1).a);
				#endif
				...
				float rim = 1 - smoothstep(min(float(0.087), rimWidth), rimWidth, viewDotNormal);
				rim *= float4(0.07417664,0.07417664,0.07417664,1).a * rimColor.a * rimMask;
				if (float(0))
				{
					switch(float(0))
					{
			...
			void ApplyRimLighting(inout PoiFragData poiFragData, in PoiMesh poiMesh, in PoiCam poiCam, in PoiLight poiLight, in PoiMods poiMods)
			{
				...
				#endif
				float3 rimColor = float3(poiThemeColor(poiMods, float4(0.07417664,0.07417664,0.07417664,1).rgb, float(0)));
			...
			float4 frag(v2f i, uint facing : SV_IsFrontFace) : SV_Target
			{
				...
				float3 mainNormal = UnpackScaleNormal(POI2D_SAMPLER_PAN(_BumpMap, _MainTex, poiUV(poiMesh.uv[float(0)], float4(1,1,0,0)), float4(0,0,0,0)), float(0.23));
