Shader "Hidden/Locked/.poiyomi/Poiyomi 8.1/Poiyomi Pro/45f3fa5b236e9f04eac225e7cdce3db7"
{
	Properties
	{
 ...
 #define _LIGHTINGMODE_FLAT 
			...
			void calculateShading(inout PoiLight poiLight, inout PoiFragData poiFragData, in PoiMesh poiMesh, in PoiCam poiCam)
			{
				...
				float roughness = 1 - (clothmapsample.a * float(0));
				float reflectance = float(0.35) * clothmapsample.b;
				...
				#else
				float roughness = 1 - (float(0));
				float metallic = pow(_ClothMetallic, 2);
				float reflectance = float(0.35);
				float clothmask = 1;
				#endif
