Shader "Hidden/Locked/.poiyomi/Poiyomi 8.1/Poiyomi Pro/ff31c35d8f64fb642bbf44a269faca3a"
{
	Properties
	{
 ...
 #define PROP_DISSOLVEDETAILNOISE 
			...
			void applyDissolve(inout PoiFragData poiFragData, in PoiMesh poiMesh, in PoiMods poiMods)
			{
				...
				if (float(1) == 1) // Basic
				{
					...
					edgeAlpha = remapClamped(da + float(0.051), da, noise) * (1 - dissolveAlpha);
				}
				...
				if (float(0.051))
				{
					edgeColor = tex2D(_DissolveEdgeGradient, poiUV(float2(edgeAlpha, edgeAlpha), float4(1,1,0,0))) * float4(poiThemeColor(poiMods, float4(0,0.1529259,1,1).rgb, float(0)), float4(0,0.1529259,1,1).a);
					#ifndef POI_SHADOW
					...
					#endif
					poiFragData.baseColor = lerp(poiFragData.baseColor, edgeColor.rgb, smoothstep(0, 1 - float(0.35) * .99999999999, edgeAlpha));
				}
				poiFragData.emission += lerp(0, dissolveToTexture * float(0), dissolveAlpha) + lerp(0, edgeColor.rgb * float(12), smoothstep(0, 1 - float(0.35) * .99999999999, edgeAlpha));
				#endif
			}
			#endif
			//ifex float(1)==0
			...
			{
				float3 x0, x1, x2;
				float3 conv = lerp(GreenWrapSH(wrap), GreenWrapSHOpt(wrap), float(0.5)); // Should try optimizing this...
			...
			void applyShadeMapping(inout PoiFragData poiFragData, PoiMesh poiMesh, inout PoiLight poiLight)
			{
				...
				mainShadowMask *= poiLight.shadowMask * float(0.75);
				firstSecondShadowMask *= poiLight.shadowMask * float(0.75);
				if (float(0) == 0)
				{
			...
			void calculateShading(inout PoiLight poiLight, inout PoiFragData poiFragData, in PoiMesh poiMesh, in PoiCam poiCam)
			{
				...
				float shadowStrength = float(0.75) * poiLight.shadowMask;
				#ifdef POI_PASS_OUTLINE
				...
				float3 directColor = _LightColor0.rgb * saturate(RTWrapFunc(poiLight.nDotL, float(2), float(0.5))) * lerp(1, poiLight.attenuation, poiLight.attenuationStrength);
				float3 indirectColor = ShadeSH9_wrapped(poiMesh.normals[float(0)], float(2)) * poiLight.occlusion;
				...
				poiLight.rampedLightMap = saturate((poiLight.lightMap - (1 - float(0.65))) / saturate((1 - float(0.15)) - (1 - float(0.65)) + fwidth(poiLight.lightMap)));
				float3 mathRamp = lerp(float3(1, 1, 1), saturate(lerp((float4(0.4479884,0.4479884,0.4479884,1) * lerp(indirectColor, 1, float(0))), float3(1, 1, 1), saturate(poiLight.rampedLightMap))), float(0.75));
				...
				poiLight.finalLighting = finalWrap * saturate(mathRamp + 1 - float(0.75));
				#endif
				#ifdef _LIGHTINGMODE_SKIN
				float3 ambientNormalWorld = poiMesh.normals[1];//aTangentToWorld(s, s.blurredNormalTangent);
				poiLight.rampedLightMap = poiLight.nDotLSaturated;
				float subsurface = 1;
				float skinScattering = saturate(subsurface * float(0.85) * 2);
				...
				poiLight.finalLighting = min(lerp(poiLight.indirectColor * float4(0.4479884,0.4479884,0.4479884,1), float4(0.4479884,0.4479884,0.4479884,1), float(0)) + (sss * poiLight.directColor), poiLight.directColor);
				#endif
				#ifdef _LIGHTINGMODE_SDF
				...
				float3 indirectColor = float4(0.4479884,0.4479884,0.4479884,1).rgb;
				indirectColor = indirectColor * lerp(poiLight.indirectColor, poiLight.directColor, float(0));
				indirectColor = lerp(poiLight.directColor, indirectColor, float(0.75) * poiLight.shadowMask);
			...
			void ApplyRimLighting(inout PoiFragData poiFragData, in PoiMesh poiMesh, in PoiCam poiCam, in PoiLight poiLight, in PoiMods poiMods)
			{
				...
				#endif
				#if defined(PROP_RIMTEX) || !defined(OPTIMIZER_ENABLED)
				float4 rimColor = POI2D_SAMPLER_PAN(_RimTex, _MainTex, poiUV(poiMesh.uv[float(0)], float4(1,1,0,0)), float4(0,0,0,0)) * float4(poiThemeColor(poiMods, float4(0.4453062,0.4453062,0.4453062,1).rgb, float(0)), float4(0.4453062,0.4453062,0.4453062,1).a);
				#else
				float4 rimColor = float4(poiThemeColor(poiMods, float4(0.4453062,0.4453062,0.4453062,1).rgb, float(0)), float4(0.4453062,0.4453062,0.4453062,1).a);
				#endif
				...
				float rim = 1 - smoothstep(min(float(0.5), rimWidth), rimWidth, viewDotNormal);
				rim *= float4(0.4453062,0.4453062,0.4453062,1).a * rimColor.a * rimMask;
				if (float(0))
				{
					switch(float(0))
					{
				...
				float3 finalRimColor = rimColor.rgb * lerp(1, poiFragData.baseColor, float(0.995));
			...
			void ApplyRimLighting(inout PoiFragData poiFragData, in PoiMesh poiMesh, in PoiCam poiCam, in PoiLight poiLight, in PoiMods poiMods)
			{
				...
				#endif
				float3 rimColor = float3(poiThemeColor(poiMods, float4(0.4453062,0.4453062,0.4453062,1).rgb, float(0)));
			...
			{
				float enviroRimAlpha = saturate(1 - smoothstep(min(float(0), float(0.5)), float(0.5), poiCam.vDotN));
				float(0.65) *= 1.7 - 0.7 * float(0.65);
				float3 enviroRimColor = 0;
				float interpolator = unity_SpecCube0_BoxMin.w;
				
				if (interpolator < 0.99999)
				{
					float4 reflectionData0 = UNITY_SAMPLE_TEXCUBE_LOD(unity_SpecCube0, poiMesh.normals[1], float(0.65) * UNITY_SPECCUBE_LOD_STEPS);
					float3 reflectionColor0 = DecodeHDR(reflectionData0, unity_SpecCube0_HDR);
					float4 reflectionData1 = UNITY_SAMPLE_TEXCUBE_SAMPLER_LOD(unity_SpecCube1, unity_SpecCube0, poiMesh.normals[1], float(0.65) * UNITY_SPECCUBE_LOD_STEPS);
				...
				{
					float4 reflectionData = UNITY_SAMPLE_TEXCUBE_LOD(unity_SpecCube0, poiMesh.normals[1], float(0.65) * UNITY_SPECCUBE_LOD_STEPS);
				...
				#endif
				float3 envRimCol = lerp(0, max(0, (enviroRimColor - float(0)) * poiFragData.baseColor), enviroRimAlpha).rgb * enviroMask * float(0.2);
			...
			float4 frag(v2f i, uint facing : SV_IsFrontFace) : SV_Target
			{
				...
				float3 mainNormal = UnpackScaleNormal(POI2D_SAMPLER_PAN(_BumpMap, _MainTex, poiUV(poiMesh.uv[float(0)], float4(10,10,0,0)), float4(0,0,0,0)), float(10));
