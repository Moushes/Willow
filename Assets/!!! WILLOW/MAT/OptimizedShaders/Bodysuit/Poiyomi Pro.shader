Shader "Hidden/Locked/.poiyomi/Poiyomi 8.1/Poiyomi Pro/ce1a3d3f91498cb4194d0db6b997392c"
{
	Properties
	{
		...
		_DissolveAlpha_Bodysuit ("Dissolve Alpha", Range(0, 1)) = 0
 ...
 #define PROP_DISSOLVENOISETEXTURE 
			...
			void applyDissolve(inout PoiFragData poiFragData, in PoiMesh poiMesh, in PoiMods poiMods)
			{
				...
				#endif
				float da = _DissolveAlpha_Bodysuit
				+ float(0)
				+ float(0)
				+ float(0)
				+ float(0)
			...
			void applyMatcap(inout PoiFragData poiFragData, in PoiCam poiCam, in PoiMesh poiMesh, inout PoiLight poiLight, in PoiMods poiMods)
			{
				...
				blendMatcap(poiLight, poiFragData, float(0.012), float(0), float(0), float(0), float(1), matcap2, matcap2Mask, float(0), float(0)
				#ifdef POI_BLACKLIGHT
			...
			void ApplyRimLighting(inout PoiFragData poiFragData, in PoiMesh poiMesh, in PoiCam poiCam, in PoiLight poiLight, in PoiMods poiMods)
			{
				...
				viewDotNormal = pow(viewDotNormal, float(0.51));
				if (float(0) && float(0))
				{
				...
				float rim = 1 - smoothstep(min(float(0.161), rimWidth), rimWidth, viewDotNormal);
			...
			float4 frag(v2f i, uint facing : SV_IsFrontFace) : SV_Target
			{
				...
				clip(poiFragData.alpha - float(0.404));
