Shader "Hidden/Locked/.poiyomi/Poiyomi Toon/e3eadcc034f46214998aab5d535e8852"
{
	Properties
	{
		...
		_EmissionHueShift_Hair ("Hue Shift", Range(0, 1)) = 0
 ...
 #define OPTIMIZER_ENABLED 
			...
			{
				poiFragData.alpha = saturate(poiFragData.alpha + -0.202);
				if (0.0 > 0)
				{
			...
			void applyDecals(inout PoiFragData poiFragData, in PoiMesh poiMesh, in PoiCam poiCam, inout PoiMods poiMods, in PoiLight poiLight)
			{
				...
				Decal0.m_DecalHueShiftEnabled = 1.0;
				...
				Decal2.m_DecalHueShiftEnabled = 1.0;
			...
			void applyMatcap(inout PoiFragData poiFragData, in PoiCam poiCam, in PoiMesh poiMesh, inout PoiLight poiLight, in PoiMods poiMods)
			{
				...
				blendMatcap(poiLight, poiFragData, poiMods, 0.0, 0.0, 0.0, 0.0, _MatcapMixed, 0.0, matcap, matcapMask, 0.0, 0.0, 0.0, 2.0, matcapALD);
				#endif
				#ifdef COLOR_GRADING_HDR_3D
				matcapALD.matcapALEnabled = 0.0;
				matcapALD.matcapALAlphaAddBand = 0.0;
				...
				blendMatcap(poiLight, poiFragData, poiMods, 0.0, 0.0, 0.0, 0.0, 0.341, 0.0, matcap2, matcap2Mask, 0.0, 0.0, 0.0, 2.0, matcapALD);
				#endif
			}
			#endif
			...
			float3 applyEmission(inout PoiFragData poiFragData, in PoiMesh poiMesh, in PoiLight poiLight, in PoiCam poiCam, in PoiMods poiMods)
			{
				...
				if (!0.0)
				{
					emissionTex = POI2D_SAMPLER_PAN(_EmissionMap, _MainTex, poiUV(poiMesh.uv[0.0], float4(4,4,0,0)), float4(0,0,0,0));
				}
				else
				{
					emissionTex = UNITY_SAMPLE_TEX2D_SAMPLER(_EmissionMap, _MainTex, ((.5 + poiLight.nDotV * .5) * float4(4,4,0,0).xy) + _Time.x * 1.0);
				}
				...
				if (0.0)
				{
					emissionStrength0 *= calculateBlinkingEmission(1.0, 2.0, 1.0, 1.0);
				}
				if(1.0)
				{
					emissionColor0 = hueShift(emissionColor0, frac(_EmissionHueShift_Hair + 0.0 * _Time.x), 0.0, 1.0);
			...
			void applyGlitter(inout PoiFragData poiFragData, in PoiMesh poiMesh, in PoiCam poiCam, in PoiLight poiLight, in PoiMods poiMods)
			{
				...
				for (uint glitterLayer = 0; glitterLayer < uint(2.0); glitterLayer++)
				{
					...
					switch(0.0)
					{
						...
						{
							randomRotation = randomFloat3WiggleRange(randoPoint, 20.0, 0.0, glitterSpeedOffset);
						}
						else
						{
						...
						finalGlitter = lerp(0, 0.1 * glitterAlpha, glitterAlpha) + max(pow(saturate(dot(lerp(glitterReflectionDirection, poiCam.viewDir, 0.43), poiCam.viewDir)), 30.0), 0);
						...
						finalGlitter = max(0.1 * glitterAlpha, brightness * glitterAlpha * smoothstep(0, 1, 1 - m_dist * 0.08 * 10));
						break;
						case 2:
						if (0.0 + glitterSpeedOffset > 0)
						{
						...
						finalGlitter = lerp(0, 0.1 * glitterAlpha, glitterAlpha) + max(pow(specAngle, 30.0), 0);
					...
					#if defined(PROP_GLITTERCOLORMAP) || !defined(OPTIMIZER_ENABLED)
					glitterColor *= POI2D_SAMPLER_PAN(_GlitterColorMap, _MainTex, poiUV(poiMesh.uv[0.0], float4(5,5,0,0)), float4(0,0,0,0)).rgb;
					#endif
					...
					#if defined(PROP_GLITTERTEXTURE) || !defined(OPTIMIZER_ENABLED)
					float4 glitterTexture = POI2D_SAMPLER_PANGRAD(_GlitterTexture, _linear_clamp, poiUV(uv, float4(5,5,0,0)), _GlitterTexturePan, poiMesh.dx, poiMesh.dy);
					#else
					...
					if (1.0)
					{
						glitterColor.rgb = hueShift(glitterColor.rgb, _GlitterHueShift + _Time.x * 0.0, 0.0, 1.0);
					}
