Shader "Hidden/Locked/.poiyomi/Poiyomi 8.1/Poiyomi Pro/eb53b5ddf304f5b4c813033769293e22"
{
	Properties
	{
 ...
 #define PROP_MOCHIEMETALLICMAPS 
			...
			void ApplyRimLighting(inout PoiFragData poiFragData, in PoiMesh poiMesh, in PoiCam poiCam, in PoiLight poiLight, in PoiMods poiMods)
			{
				...
				float blendStrength = float(0.546);
				#ifdef POI_AUDIOLINK
				...
				#endif
				#if defined(PROP_RIMTEX) || !defined(OPTIMIZER_ENABLED)
				float4 rimColor = POI2D_SAMPLER_PAN(_RimTex, _MainTex, poiUV(poiMesh.uv[float(0)], float4(1,1,0,0)), float4(0,0,0,0)) * float4(poiThemeColor(poiMods, float4(0.03864442,0.03864442,0.03864442,1).rgb, float(0)), float4(0.03864442,0.03864442,0.03864442,1).a);
				#else
				float4 rimColor = float4(poiThemeColor(poiMods, float4(0.03864442,0.03864442,0.03864442,1).rgb, float(0)), float4(0.03864442,0.03864442,0.03864442,1).a);
				#endif
				...
				rim *= float4(0.03864442,0.03864442,0.03864442,1).a * rimColor.a * rimMask;
				if (float(0))
				{
					switch(float(0))
					{
			...
			void ApplyRimLighting(inout PoiFragData poiFragData, in PoiMesh poiMesh, in PoiCam poiCam, in PoiLight poiLight, in PoiMods poiMods)
			{
				...
				#endif
				float3 rimColor = float3(poiThemeColor(poiMods, float4(0.03864442,0.03864442,0.03864442,1).rgb, float(0)));
			...
			void applyGlitter(inout PoiFragData poiFragData, in PoiMesh poiMesh, in PoiCam poiCam, in PoiLight poiLight, in PoiMods poiMods)
			{
				...
				switch(float(0))
				{
					...
					finalGlitter = lerp(0, float(0) * glitterAlpha, glitterAlpha) + max(pow(saturate(dot(lerp(glitterReflectionDirection, poiCam.viewDir, float(0)), poiCam.viewDir)), float(300)), 0);
				...
				if (float(0) == 1)
				{
					poiFragData.baseColor = lerp(poiFragData.baseColor, finalGlitter * glitterColor * float(29.1), finalGlitter * glitterTexture.a * glitterMask);
					poiFragData.emission += finalGlitter * glitterColor * max(0, (float(29.1) - 1) * glitterTexture.a) * glitterMask;
				}
				else
				{
					poiFragData.emission += finalGlitter * glitterColor * float(29.1) * glitterTexture.a * glitterMask;
				}
			}
			#endif
			//ifex float(1)==0 && float(0)==0
			...
			void MochieBRDF(inout PoiFragData poiFragData, in PoiCam poiCam, inout PoiLight poiLight, in PoiMesh poiMesh, in PoiMods poiMods)
			{
				...
				#endif
				reflectionMask *= float(0.259);
				specularMask *= float(2.05);
				if (float(0))
				{
			...
			{
				float enviroRimAlpha = saturate(1 - smoothstep(min(float(0.341), float(1)), float(1), poiCam.vDotN));
				float(0.618) *= 1.7 - 0.7 * float(0.618);
				float3 enviroRimColor = 0;
				float interpolator = unity_SpecCube0_BoxMin.w;
				
				if (interpolator < 0.99999)
				{
					float4 reflectionData0 = UNITY_SAMPLE_TEXCUBE_LOD(unity_SpecCube0, poiMesh.normals[1], float(0.618) * UNITY_SPECCUBE_LOD_STEPS);
					float3 reflectionColor0 = DecodeHDR(reflectionData0, unity_SpecCube0_HDR);
					float4 reflectionData1 = UNITY_SAMPLE_TEXCUBE_SAMPLER_LOD(unity_SpecCube1, unity_SpecCube0, poiMesh.normals[1], float(0.618) * UNITY_SPECCUBE_LOD_STEPS);
				...
				{
					float4 reflectionData = UNITY_SAMPLE_TEXCUBE_LOD(unity_SpecCube0, poiMesh.normals[1], float(0.618) * UNITY_SPECCUBE_LOD_STEPS);
				...
				#endif
				float3 envRimCol = lerp(0, max(0, (enviroRimColor - float(0.058)) * poiFragData.baseColor), enviroRimAlpha).rgb * enviroMask * float(1);
			...
			float4 frag(v2f i, uint facing : SV_IsFrontFace) : SV_Target
			{
				...
				float3 mainNormal = UnpackScaleNormal(POI2D_SAMPLER_PAN(_BumpMap, _MainTex, poiUV(poiMesh.uv[float(0)], float4(1,1,0,0)), float4(0,0,0,0)), _BumpScale);
