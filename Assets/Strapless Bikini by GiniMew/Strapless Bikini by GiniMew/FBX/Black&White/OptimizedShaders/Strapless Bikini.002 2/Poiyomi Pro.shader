Shader "Hidden/Locked/.poiyomi/Poiyomi 8.1/Poiyomi Pro/2adddadefcb2a464eaa98d76e7d07564"
{
	Properties
	{
 ...
 #define _RIMSTYLE_POIYOMI 
			...
			void ApplyRimLighting(inout PoiFragData poiFragData, in PoiMesh poiMesh, in PoiCam poiCam, in PoiLight poiLight, in PoiMods poiMods)
			{
				...
				#endif
				#if defined(PROP_RIMTEX) || !defined(OPTIMIZER_ENABLED)
				float4 rimColor = POI2D_SAMPLER_PAN(_RimTex, _MainTex, poiUV(poiMesh.uv[float(0)], float4(1,1,0,0)), float4(0,0,0,0)) * float4(poiThemeColor(poiMods, float4(0.002190549,0.002190549,0.002190549,1).rgb, float(0)), float4(0.002190549,0.002190549,0.002190549,1).a);
				#else
				float4 rimColor = float4(poiThemeColor(poiMods, float4(0.002190549,0.002190549,0.002190549,1).rgb, float(0)), float4(0.002190549,0.002190549,0.002190549,1).a);
				#endif
				...
				rim *= float4(0.002190549,0.002190549,0.002190549,1).a * rimColor.a * rimMask;
				if (float(0))
				{
					switch(float(0))
					{
			...
			void ApplyRimLighting(inout PoiFragData poiFragData, in PoiMesh poiMesh, in PoiCam poiCam, in PoiLight poiLight, in PoiMods poiMods)
			{
				...
				#endif
				float3 rimColor = float3(poiThemeColor(poiMods, float4(0.002190549,0.002190549,0.002190549,1).rgb, float(0)));
			...
			void ApplyDepthRimLighting(inout PoiFragData poiFragData, in PoiMesh poiMesh, in PoiCam poiCam, inout PoiLight poiLight, in PoiMods poiMods)
			{
				...
				switch(float(0))
				{
					case 0:
					{
						...
						float3 viewNorm = mul((float3x3)UNITY_MATRIX_V, poiMesh.normals[float(1)]);
						...
						float scale = float(0.2) * GetSSRimScale(depth);
						...
						rim = smoothstep(0.24 * float(0.2) * z, 0.25 * z, depthDiff);
						rim *= lerp(1, (dot(L_View, N_View) > 0), float(0));
					}
					break;
					case 1:
					{
						...
						rim = smoothstep(0.24 * float(0.2) * z, 0.25 * z, depthDiff);
						rim *= lerp(1, lerp(poiLight.vertexNDotL > 0, poiLight.nDotL > 0, float(1)), float(0));
					}
					break;
				}
				float3 rimColor = poiThemeColor(poiMods, float4(1,1,1,1).rgb, float(0)).rgb * lerp(1, poiLight.directColor, float(0)) * lerp(1, poiFragData.baseColor, float(0)) * float(1);
				poiLight.finalLightAdd += rim * rimColor * float(0);
				poiFragData.emission += rim * rimColor * float(0);
				poiFragData.baseColor = lerp(poiFragData.baseColor, rimColor, rim * float(0));
				poiFragData.baseColor += rim * rimColor * float(0);
				poiFragData.baseColor *= lerp(1, rimColor, rim * float(0));
			}
			#endif
