Shader "Hidden/Locked/.poiyomi/Poiyomi Toon/b4a9ff18a2965db48a9189f9fd8936a1"
{
	Properties
	{
 ...
 #define OPTIMIZER_ENABLED 
			...
			{
				poiFragData.alpha = saturate(poiFragData.alpha + 0.135);
				if (0.0 > 0)
				{
			...
			float4 frag(VertexOut i, uint facing : SV_IsFrontFace) : SV_Target
			{
				...
				poiFragData.baseColor = lerp(poiFragData.baseColor, dot(poiFragData.baseColor, float3(0.3, 0.59, 0.11)), - (-0.87) * hueShiftAlpha.b);
				poiFragData.baseColor = saturate(lerp(poiFragData.baseColor, poiFragData.baseColor * (-0.755 + 1), hueShiftAlpha.g));
				#endif
				if (2.0)
				{
					#if defined(PROP_ALPHAMASK) || !defined(OPTIMIZER_ENABLED)
