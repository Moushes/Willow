Shader "Hidden/Locked/.poiyomi/Poiyomi 8.1/Poiyomi Pro Outline/e4d92024ebac2bb40bb7169d97538425"
{
	Properties
	{
 ...
 #define PROP_MATCAP3MASK 
			...
			float _AlphaBoostFA;
			//ifex float(0)==0 && float(0)==0 && float(1)==0 && float(0)==0
			...
			{
				poiFragData.alpha = saturate(poiFragData.alpha + float(-0.416));
			}
			...
			void applyDecals(inout PoiFragData poiFragData, in PoiMesh poiMesh, in PoiCam poiCam, in PoiMods poiMods, in PoiLight poiLight)
			{
				...
				decalColor = tex2D(_DecalTexture2, uv + float4(0,0,0,0) * _Time.x, ddxuv, ddyuv) * float4(poiThemeColor(poiMods, float4(0,0,0,1).rgb, float(0)), float4(0,0,0,1).a);
				#else
				uv = decalUV(float(0), float4(1,1,0,0), float4(0.5,0.5,0,0), float(0) + decalRotation, float(0), decalScale, float4(0,0,0,0) + sideMod, float(0), poiMesh, poiCam);
				decalColor = float4(poiThemeColor(poiMods, float4(0,0,0,1).rgb, float(0)), float4(0,0,0,1).a);
				#endif
			...
			#endif
			//ifex float(1)==0 && float(0)==0 && float(1)==0 && float(0)==0
			...
			void applyMatcap(inout PoiFragData poiFragData, in PoiCam poiCam, in PoiMesh poiMesh, inout PoiLight poiLight, in PoiMods poiMods)
			{
				...
				if (float(1))
				{
					matcap3.rgb = hueShift(matcap3.rgb, _Matcap3HueShift + _Time.x * float(0));
				}
				blendMatcap(poiLight, poiFragData, _Matcap3Add, float(0), float(0), _Matcap3Replace, float(0), matcap3, matcap3Mask, float(0), float(0)
				#ifdef POI_BLACKLIGHT
			...
			float4 frag(v2f i, uint facing : SV_IsFrontFace) : SV_Target
			{
				...
				float3 mainNormal = UnpackScaleNormal(POI2D_SAMPLER_PAN(_BumpMap, _MainTex, poiUV(poiMesh.uv[float(0)], float4(1,1,0,0)), float4(0,0,0,0)), float(3.18));
				...
				clip(poiFragData.alpha - float(0.502));
