Shader "Hidden/Locked/.poiyomi/Poiyomi 8.1/Poiyomi Pro/e3eadcc034f46214998aab5d535e8852"
{
	Properties
	{
		...
		_DecalBlendAlpha_Hair ("Alpha", Range(0, 1)) = 1
		...
		_DecalBlendAlpha1_Hair ("Alpha", Range(0, 1)) = 1
		...
		_DecalBlendAlpha2_Hair ("Alpha", Range(0, 1)) = 1
		...
		_DecalBlendAlpha3_Hair ("Alpha", Range(0, 1)) = 1
		...
		_EmissionHueShift_Hair ("Hue Shift--{condition_showS:(_EmissionHueShiftEnabled==1)}", Range(0, 1)) = 0
 ...
 #define PROP_GLITTERMASK 
			...
			{
				poiFragData.alpha = saturate(poiFragData.alpha + float(-0.202));
			}
			//ifex float(1)==0 && isNotAnimated(float(1))
			...
			void applyDecals(inout PoiFragData poiFragData, in PoiMesh poiMesh, in PoiCam poiCam, in PoiMods poiMods, in PoiLight poiLight)
			{
				...
				#endif
				decalColor.rgb = decalHueShift(float(1), decalColor.rgb, _DecalHueShift + poiLight.nDotV * float(0), float(0));
				...
				#endif
				poiFragData.baseColor.rgb = lerp(poiFragData.baseColor.rgb, customBlend(poiFragData.baseColor.rgb, decalColor.rgb, float(0)), decalColor.a * saturate(_DecalBlendAlpha_Hair + audioLinkDecalAlpha0));
				if (float(0))
				{
					alphaOverride += 1;
				...
				decalColor = float4(poiThemeColor(poiMods, float4(1,1,1,1).rgb, float(0)), float4(1,1,1,1).a);
				#endif
				decalColor.rgb = decalHueShift(float(1), decalColor.rgb, _DecalHueShift1 + poiLight.nDotV * float(0), float(0));
				...
				#endif
				poiFragData.baseColor.rgb = lerp(poiFragData.baseColor.rgb, customBlend(poiFragData.baseColor.rgb, decalColor.rgb, float(0)), decalColor.a * saturate(_DecalBlendAlpha1_Hair + audioLinkDecalAlpha1));
				if (float(0))
				{
					alphaOverride += 1;
				...
				#endif
				poiFragData.baseColor.rgb = lerp(poiFragData.baseColor.rgb, customBlend(poiFragData.baseColor.rgb, decalColor.rgb, float(0)), decalColor.a * saturate(_DecalBlendAlpha2_Hair + audioLinkDecalAlpha2));
				if (float(0))
				{
					alphaOverride += 1;
				...
				#endif
				poiFragData.baseColor.rgb = lerp(poiFragData.baseColor.rgb, customBlend(poiFragData.baseColor.rgb, decalColor.rgb, float(0)), decalColor.a * saturate(_DecalBlendAlpha3_Hair + audioLinkDecalAlpha3));
				if (float(0))
				{
					alphaOverride += 1;
			...
			void applyMatcap(inout PoiFragData poiFragData, in PoiCam poiCam, in PoiMesh poiMesh, inout PoiLight poiLight, in PoiMods poiMods)
			{
				...
				blendMatcap(poiLight, poiFragData, float(0), float(0), float(0), float(0), _MatcapMixed, matcap, matcapMask, float(0), float(0)
				#ifdef POI_BLACKLIGHT
				...
				blendMatcap(poiLight, poiFragData, float(0), float(0), float(0), float(0), float(0.341), matcap2, matcap2Mask, float(0), float(0)
				#ifdef POI_BLACKLIGHT
			...
			float3 applyEmission(inout PoiFragData poiFragData, in PoiMesh poiMesh, in PoiLight poiLight, in PoiCam poiCam, in PoiMods poiMods)
			{
				...
				if (!float(0))
				{
					emissionColor0 = POI2D_SAMPLER_PAN(_EmissionMap, _MainTex, poiUV(poiMesh.uv[float(0)], float4(4,4,0,0)), float4(0,0,0,0)).rgb * lerp(1, poiFragData.baseColor, float(0)).rgb * poiThemeColor(poiMods, _EmissionColor.rgb, float(0));
				}
				else
				{
					emissionColor0 = UNITY_SAMPLE_TEX2D_SAMPLER(_EmissionMap, _MainTex, ((.5 + poiLight.nDotV * .5) * float4(4,4,0,0).xy) + _Time.x * float(1)).rgb * lerp(1, poiFragData.baseColor, float(0)).rgb * poiThemeColor(poiMods, _EmissionColor.rgb, float(0));
				}
				#else
				emissionColor0 = lerp(1, poiFragData.baseColor, float(0)).rgb * poiThemeColor(poiMods, _EmissionColor.rgb, float(0));
				#endif
				if (float(0))
				{
				...
				if (float(0))
				{
					emissionStrength0 *= calculateBlinkingEmission(float(1), float(2), float(1), float(1));
				}
				applyLumaGradient(poiMods, emissionColor0, float(0), poiLight.nDotV);
				emissionColor0 = hueShift(emissionColor0, frac(_EmissionHueShift_Hair + float(0) * _Time.x) * float(1));
				#if defined(PROP_EMISSIONMASK) || !defined(OPTIMIZER_ENABLED)
			...
			{
				float2 st = frac(poiMesh.uv[float(0)] + float4(0,0,0,0).xy * _Time.x) * float(800);
				...
				switch(float(0))
				{
					...
					if (float(0) > 0)
					{
						randomRotation = randomFloat3WiggleRange(randoPoint, float(20), float(0));
					}
					else
					{
						randomRotation = randomFloat3Range(randoPoint, float(20));
					}
					float3 glitterReflectionDirection = normalize(mul(poiRotationMatrixFromAngles(randomRotation), norm));
					finalGlitter = lerp(0, float(0.1) * glitterAlpha, glitterAlpha) + max(pow(saturate(dot(lerp(glitterReflectionDirection, poiCam.viewDir, float(0.43)), poiCam.viewDir)), float(30)), 0);
					finalGlitter *= glitterAlpha;
					break;
					case 1:
					float offset = random(randoPoint);
					float brightness = sin((_Time.x + offset) * float(0)) * float(20) - (float(20) - 1);
					finalGlitter = max(float(0.1) * glitterAlpha, brightness * glitterAlpha * smoothstep(0, 1, 1 - m_dist * float(0.08) * 10));
					break;
					case 2:
					if (float(0) > 0)
					{
					...
					finalGlitter = lerp(0, float(0.1) * glitterAlpha, glitterAlpha) + max(pow(specAngle, float(30)), 0);
				...
				#if defined(PROP_GLITTERCOLORMAP) || !defined(OPTIMIZER_ENABLED)
				glitterColor *= POI2D_SAMPLER_PAN(_GlitterColorMap, _MainTex, poiUV(poiMesh.uv[float(0)], float4(5,5,0,0)), float4(0,0,0,0)).rgb;
				#endif
				...
				#if defined(PROP_GLITTERTEXTURE) || !defined(OPTIMIZER_ENABLED)
				float4 glitterTexture = POI2D_SAMPLER_PAN(_GlitterTexture, _MainTex, poiUV(uv, float4(5,5,0,0)), float4(0,0,0,0));
				#else
				...
				if (float(1))
				{
					glitterColor.rgb = hueShift(glitterColor.rgb, _GlitterHueShift + _Time.x * float(0));
				}
