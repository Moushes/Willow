Shader "Hidden/Locked/.poiyomi/Poiyomi 8.1/Poiyomi Pro/7e9fefc2a4eeba94eacf343d8db61f4b"
{
	Properties
	{
		...
		_DissolveAlpha_Top ("Dissolve Alpha", Range(0, 1)) = 0
 ...
 #define PROP_DISSOLVENOISETEXTURE 
			...
			void applyDecals(inout PoiFragData poiFragData, in PoiMesh poiMesh, in PoiCam poiCam, in PoiMods poiMods, in PoiLight poiLight)
			{
				...
				uv = decalUV(float(0), float4(1,1,0,0), float4(0.5,0.5,0,0), float(360) + decalRotation, float(0), decalScale, float4(0,0,0,0) + sideMod, float(0), poiMesh, poiCam);
				ddxuv = ddx(uv);
				ddyuv = ddy(uv);
				if (any(fwidth(uv) > .5))
				{
			...
			void ApplyRimLighting(inout PoiFragData poiFragData, in PoiMesh poiMesh, in PoiCam poiCam, in PoiLight poiLight, in PoiMods poiMods)
			{
				...
				viewDotNormal = pow(viewDotNormal, float(0.81));
				if (float(0) && float(0))
				{
				...
				#endif
				#if defined(PROP_RIMTEX) || !defined(OPTIMIZER_ENABLED)
				float4 rimColor = POI2D_SAMPLER_PAN(_RimTex, _MainTex, poiUV(poiMesh.uv[float(0)], float4(1,1,0,0)), float4(0,0,0,0)) * float4(poiThemeColor(poiMods, float4(0.08950669,0.09176325,0.09992067,1).rgb, float(0)), float4(0.08950669,0.09176325,0.09992067,1).a);
				#else
				float4 rimColor = float4(poiThemeColor(poiMods, float4(0.08950669,0.09176325,0.09992067,1).rgb, float(0)), float4(0.08950669,0.09176325,0.09992067,1).a);
				#endif
				...
				rim *= float4(0.08950669,0.09176325,0.09992067,1).a * rimColor.a * rimMask;
				if (float(0))
				{
					switch(float(0))
					{
			...
			void ApplyRimLighting(inout PoiFragData poiFragData, in PoiMesh poiMesh, in PoiCam poiCam, in PoiLight poiLight, in PoiMods poiMods)
			{
				...
				#endif
				float3 rimColor = float3(poiThemeColor(poiMods, float4(0.08950669,0.09176325,0.09992067,1).rgb, float(0)));
