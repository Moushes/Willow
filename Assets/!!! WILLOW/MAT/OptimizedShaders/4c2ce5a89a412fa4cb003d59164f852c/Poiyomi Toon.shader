Shader "Hidden/Locked/.poiyomi/Poiyomi Toon/4c2ce5a89a412fa4cb003d59164f852c"
{
	Properties
	{
		...
		[HideInInspector] s_start_MainHueShift_Eye_R ("Hue Shift--{reference_property:_MainHueShiftToggle,persistent_expand:true,default_expand:true}", Float) = 1
		...
		_MainHueShift_Eye_R ("Hue Shift", Range(0, 1)) = 0
		...
		[HideInInspector] s_end_MainHueShift_Eye_R ("Name Motion", Float) = 0
 ...
 #define OPTIMIZER_ENABLED 
			...
			void applyGlitter(inout PoiFragData poiFragData, in PoiMesh poiMesh, in PoiCam poiCam, in PoiLight poiLight, in PoiMods poiMods)
			{
				...
				{
					float2 st = (poiMesh.uv[0.0] + float4(0,0,0,0).xy * _Time.x) * 500.0;
					...
					switch(0.0)
					{
						...
						{
							randomRotation = randomFloat3WiggleRange(randoPoint, 31.0, 10.0, glitterSpeedOffset);
						}
						else
						{
							randomRotation = randomFloat3Range(randoPoint, 31.0);
						}
						float3 glitterReflectionDirection = normalize(mul(poiRotationMatrixFromAngles(randomRotation), norm));
						finalGlitter = lerp(0, 0.0 * glitterAlpha, glitterAlpha) + max(pow(saturate(dot(lerp(glitterReflectionDirection, poiCam.viewDir, 0.4), poiCam.viewDir)), 30.0), 0);
						...
						finalGlitter = lerp(0, 0.0 * glitterAlpha, glitterAlpha) + max(pow(specAngle, 30.0), 0);
						glitterColor *= poiLight.directColor;
						finalGlitter *= glitterAlpha;
						break;
					}
					glitterColor *= lerp(1, poiFragData.baseColor, 0.999);
					#if defined(PROP_GLITTERCOLORMAP) || !defined(OPTIMIZER_ENABLED)
					...
					if (0.0 == 1)
					{
						poiFragData.baseColor = lerp(poiFragData.baseColor, finalGlitter * glitterColor * (5.0 + GlitterbrightnessOffset), finalGlitter * glitterTexture.a * glitterMask);
						poiFragData.emission += finalGlitter * glitterColor * max(0, ((5.0 + GlitterbrightnessOffset) - 1) * glitterTexture.a) * glitterMask;
					}
					else
					{
						poiFragData.emission += finalGlitter * glitterColor * (5.0 + GlitterbrightnessOffset) * glitterTexture.a * glitterMask;
					}
				}
			}
			#endif
