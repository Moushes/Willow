Shader "Hidden/Locked/.poiyomi/Poiyomi 8.1/Poiyomi Pro/1c24b33d734beeb4eb186030102364c3"
{
	Properties
	{
 ...
 #define PROP_DISSOLVENOISETEXTURE 
			...
			void applyMatcap(inout PoiFragData poiFragData, in PoiCam poiCam, in PoiMesh poiMesh, inout PoiLight poiLight, in PoiMods poiMods)
			{
				...
				#if defined(PROP_MATCAP) || !defined(OPTIMIZER_ENABLED)
				matcap = UNITY_SAMPLE_TEX2D_SAMPLER(_Matcap, _MainTex, TRANSFORM_TEX(matcapUV, _Matcap)) * float4(poiThemeColor(poiMods, float4(0.1887741,0.1887741,0.1887741,1).rgb, float(0)), float4(0.1887741,0.1887741,0.1887741,1).a);
				#else
				matcap = float4(poiThemeColor(poiMods, float4(0.1887741,0.1887741,0.1887741,1).rgb, float(0)), float4(0.1887741,0.1887741,0.1887741,1).a);
				#endif
				matcap.rgb *= float(0.48);
				#if defined(PROP_MATCAPMASK) || !defined(OPTIMIZER_ENABLED)
				...
				#endif
				poiFragData.alpha *= lerp(1, matcap.a, matcapMask * float(1));
				if (float(0))
				{
					matcap.rgb = hueShift(matcap.rgb, float(0) + _Time.x * float(0));
				}
				blendMatcap(poiLight, poiFragData, float(0.031), float(0), float(1), float(0.597), float(1), matcap, matcapMask, float(0), float(0.52)
				#ifdef POI_BLACKLIGHT
			...
			void ApplyRimLighting(inout PoiFragData poiFragData, in PoiMesh poiMesh, in PoiCam poiCam, in PoiLight poiLight, in PoiMods poiMods)
			{
				...
				viewDotNormal = pow(viewDotNormal, float(1.11));
				if (float(0) && float(0))
				{
				...
				float blendStrength = float(0.29);
				#ifdef POI_AUDIOLINK
			...
			float4 frag(v2f i, uint facing : SV_IsFrontFace) : SV_Target
			{
				...
				float3 mainNormal = UnpackScaleNormal(POI2D_SAMPLER_PAN(_BumpMap, _MainTex, poiUV(poiMesh.uv[float(0)], float4(32,32,0,0)), float4(0,0,0,0)), float(1));
